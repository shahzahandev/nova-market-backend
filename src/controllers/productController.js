  ...
  try {
    ...
    if (specifications) {
      try {
        ...
        parsedSpecifications = parsedSpecifications
          .filter(
            (spec) =>
              spec &&
              spec.name &&
    ...
    const mainIndex =
      Number.isInteger(Number(isMain))
        ? Number(isMain)
        : 0;
    ...
    // ==================================================
    // MAKE SURE THERE IS A MAIN IMAGE
    // ==================================================
    ...
    if (existingSku) {
      return res.status(400).json({
        success: false,
        message:
          "A product with this SKU already exists. Please use a different product title.",
      });
    }
    ...
    const product = new Product({
      ...
      discountStartDate:
        hasDiscount
          ? finalDiscountStartDate
          : undefined,

      discountEndDate:
        hasDiscount
          ? finalDiscountEndDate
          : undefined,
...
exports.updateProductController = async (
  ...
  try {
    ...
    if (
      req.body.discountStartDate !== undefined
    ) {
    ...
    if (
      req.body.discountEndDate !== undefined
    ) {
    ...
    if (
      ...
      for (const image of product.images) {
        ...
        try {
          ...
          // Cloudinary public_id থাকলে delete করবো
          if (image.public_id) {
